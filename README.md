...
**Τιμή και "Πουλήθηκε" σε κάθε έργο.** Στο `admin.html` υπάρχει τρίτη καρτέλα,
**Edit price / sold**: διάλεξε ένα υπάρχον έργο από τη λίστα, βάλε κόστος
υλικών/εργασίας/μεταφοράς και επίλεξε αν η κορνίζα είναι χωρίς/υποχρεωτική/
προαιρετική (ίδια λογική με το `pricing.html`), ή απλά τσέκαρε **Mark this
piece as sold**, και πάτα **Save changes**. Στο site, κάθε έργο με τιμή
εμφανίζει μια ετικέτα τιμής με σύνδεσμο **"Price breakdown"** που ανοίγει την
αναλυτική ανάλυση (με σύγκριση με/χωρίς κορνίζα όταν είναι προαιρετική)· κάθε
έργο με "sold" παίρνει μια κόκκινη ετικέτα **SOLD** πάνω στη φωτογραφία. Το
ίδιο πεδίο τιμής υπάρχει και στην καρτέλα **New artwork**, αν θες να ορίσεις
τιμή κατευθείαν όταν προσθέτεις κάτι καινούργιο.
...
## 4. Τιμολόγηση: το εργαλείο "Price Quotation" (pricing.html)

Ένα τρίτο σελιδάκι — `pricing.html` — φτιάχνει ένα επαγγελματικό PDF με την
αναλυτική τιμολόγηση ενός έργου, έτοιμο να το στείλεις σε πελάτη που
ενδιαφέρεται να αγοράσει. Δουλεύει εξ ολοκλήρου μέσα στον browser (δεν
ανεβάζει τίποτα πουθενά) — απλώς κατεβάζει ένα PDF στη συσκευή σου.

**Πώς το χρησιμοποιείς:**

1. Άνοιξέ το στο `https://το-domain-σου/pricing.html` (ή τοπικά μέσω του
   `index.html` σου, αν τρέχεις έναν local server· βλ. ενότητα 6 — απευθείας
   διπλό-κλικ δεν φορτώνει τη λίστα έργων, επειδή ο browser μπλοκάρει τοπικά
   αρχεία JSON, αλλά μπορείς πάντα να συμπληρώσεις τίτλο/υλικό/διαστάσεις με
   το χέρι).
2. Διάλεξε το έργο από τη λίστα (ή γράψε τα στοιχεία του με το χέρι, αν δεν
   είναι ακόμα στο site) — η φωτογραφία του έργου φορτώνει αυτόματα και
   εμφανίζεται μέσα στο PDF, δίπλα στον τίτλο. Μπορείς να ανεβάσεις άλλη
   φωτογραφία αν θέλεις (π.χ. καλύτερη λήψη, ή για ένα καινούργιο έργο).
3. Συμπλήρωσε τον αγοραστή (προαιρετικό), το κόστος υλικών, εργασίας και
   μεταφοράς.
4. Για την κορνίζα διάλεξε ένα από τα τρία: **χωρίς κορνίζα** (καμία γραμμή
   κορνίζας, μία τιμή), **υποχρεωτική κορνίζα** (μία τιμή, με την κορνίζα
   μέσα), ή **προαιρετική κορνίζα** (εμφανίζονται *και οι δύο* τιμές — με και
   χωρίς — ώστε ο πελάτης να διαλέξει).
5. Πάτα **Generate quotation** — θα δεις προεπισκόπηση του PDF από κάτω, και
   μετά **Download PDF** για να το κατεβάσεις. Η υπογραφή σου μπαίνει
   αυτόματα στο κάτω μέρος κάθε φορά.

Το PDF χρησιμοποιεί την ίδια γραμματοσειρά και χρώματα με το site, οπότε
δείχνει σαν φυσική προέκταση της δουλειάς σου — όχι σαν γενική φόρμα.
...
pricing.html            ← η φόρμα τιμολόγησης / PDF (βλ. ενότητα 4)
data/
  works.json          ← όλα τα έργα σου
  exhibitions.json    ← όλες οι εκθέσεις σου
images/works/         ← οι φωτογραφίες των έργων
images/signature.png  ← η υπογραφή σου (χρησιμοποιείται από το pricing.html)
src/
...
## 6. Δοκιμή στον υπολογιστή σου (προαιρετικό)
