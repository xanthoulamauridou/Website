...
## 2. Ο εύκολος τρόπος: το εργαλείο "Add Work" (admin.html)

Στο project υπάρχει ένα δεύτερο, ξεχωριστό σελιδάκι — `admin.html` — μια απλή
φόρμα: ανεβάζεις φωτογραφία, γράφεις τίτλο/υλικό/διαστάσεις, πατάς **Publish**,
και αυτό δημιουργεί μόνο του τα σωστά commits στο GitHub (ανεβάζει τη φωτογραφία
+ ενημερώνει το `data/works.json` ή `data/exhibitions.json`). Δεν χρειάζεται να
ανοίξεις ποτέ JSON αρχείο με το χέρι.

**Πώς το χρησιμοποιείς:**

1. Άνοιξέ το είτε τοπικά (διπλό κλικ στο `admin.html`) είτε online στο δικό σου
   domain, στη διεύθυνση `https://το-domain-σου/admin.html` — δουλεύει και από
   το κινητό.
2. **Φτιάξε ένα GitHub token** (μία φορά ανά συσκευή): πήγαινε
   [github.com/settings/personal-access-tokens/new](https://github.com/settings/personal-access-tokens/new) →
   **Fine-grained token** → στο "Repository access" επίλεξε **Only select
   repositories** και διάλεξε το repo σου → στο "Permissions" δώσε στο
   **Contents** δικαίωμα **Read and write** → Generate token → αντέγραψέ το
   (ξεκινάει με `github_pat_...`).
3. Στο `admin.html`, βάλε το GitHub username σου, το όνομα του repository, και
   κόλλησε το token. Πάτα **Test connection** για επιβεβαίωση.
   - Το token αποθηκεύεται **μόνο μέσα σε αυτόν τον browser** (localStorage) —
     δεν στέλνεται πουθενά αλλού εκτός από κατευθείαν στο GitHub. Μην το
     χρησιμοποιήσεις σε δημόσιο/κοινόχρηστο υπολογιστή, και μπορείς να το
     ανακαλέσεις οποιαδήποτε στιγμή από το GitHub αν χρειαστεί.
4. Επίλεξε **New artwork** ή **New exhibition**, συμπλήρωσε τα πεδία, διάλεξε
   φωτογραφία/ες, πάτα **Publish**. Μέσα σε 1-2 λεπτά το βλέπεις live.

Το εργαλείο μικραίνει αυτόματα τις φωτογραφίες πριν τις ανεβάσει, οπότε μπορείς
να ανεβάζεις κατευθείαν φωτό από κινητό χωρίς να ανησυχείς για το μέγεθος.

---

## 3. Ο χειροκίνητος τρόπος (προαιρετικός)

Αν προτιμάς να επεξεργάζεσαι τα αρχεία μόνη σου μέσα από το GitHub (π.χ. για
πιο σύνθετες περιπτώσεις, όπως μια σειρά έργων με κοινό `seriesNote`), μπορείς
να συνεχίσεις να κάνεις ό,τι περιγράφεται παρακάτω.

### Πώς προσθέτεις ένα νέο έργο
...
admin.html              ← η φόρμα "Add Work" (βλ. ενότητα 2)
data/
